    ...
    pub fn message(&self) -> String {
        ...
        match self {
            ...
            SessionEvent::TurnCompleted { elapsed: None } => {
                locale.tr("Turn completed.").into_owned()
            }
            SessionEvent::TurnCancelled { elapsed, cause } => {
                let duration = format_duration(*elapsed);
                // The cause is a fragment upstream composes into the sentence, so it
                // is localized on its own and then substituted as the `{cause}` value.
                let phrase = locale.tr_static(cause.phrase());
                locale.tr_format("{cause} in {duration}.",
                    &[("cause", phrase), ("duration", &duration)],
                )
            }
            ...
            SessionEvent::CompactionStarted { percentage, reason } => {
                if reason == MODEL_FAMILY_SWITCH_COMPACT_BANNER {
                    ...
                    locale.tr_format("Context {percentage}% full. Compacting…",
