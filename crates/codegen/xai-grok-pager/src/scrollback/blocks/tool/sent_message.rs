            ...
            Self::Unresolved { subagent_id } => {
                ...
                match id.char_indices().nth_back(SHORT_ID_CHARS) {
                    ...
                    Some((index, ch)) => {
                        ...
                        Cow::Owned(format!("{noun} \u{2026}{tail}"))
                    }
                    None if id.is_empty() => Cow::Borrowed(noun),
                    None => Cow::Owned(format!("{noun} {id}")),
                }
            }
        }
    }
    ...
    fn verb_and_target(&self) -> String {
        ...
        match &self.presentation {
            SentMessagePresentation::Sending => {
                locale.tr_format("sending to {noun}", &[("noun", noun)])
            }
            SentMessagePresentation::Sent => {
                match self.input.as_ref().and_then(|input| input.delivery) {
                    None | Some(SentMessageDelivery::Steer) => {
                        locale.tr_format("sent to {noun}", &[("noun", noun)])
                    }
                    Some(SentMessageDelivery::Queue) => {
                        locale.tr_format("queued for {noun}", &[("noun", noun)])
                    }
                    Some(SentMessageDelivery::Interject) => {
                        locale.tr_format("interjected to {noun}", &[("noun", noun)])
                    }
                }
            }
            SentMessagePresentation::Rejected { .. } => {
                locale.tr_format("rejected \u{00b7} {noun}", &[("noun", noun)])
            }
            SentMessagePresentation::Unconfirmed { .. } => {
                locale.tr_format("unconfirmed \u{00b7} {noun}", &[("noun", noun)])
            }
        }
    }
    ...
    fn output(&self, ctx: &BlockContext) -> BlockOutput {
        ...
        if let Some(raw_id) = self.input.as_ref().and_then(|input| input.target.raw_id()) {
            ...
            let id_wrap = RtOptions::new(width).initial_indent(Line::from(Span::styled(
                crate::locale::ctx().tr("Subagent ID: ").into_owned(),
                theme.muted(),
            )));
        ...
        lines.push(BlockLine::separator(Line::from(Span::styled(
            crate::locale::ctx().tr("Message:").into_owned(),
            theme.muted(),
        ))));
