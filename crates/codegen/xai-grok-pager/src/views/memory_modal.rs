    ...
    fn load_preview(&mut self) {
        ...
        self.preview_markdown = path.and_then(|path| {
            ...
            if bytes.len() as u64 > MAX_PREVIEW_BYTES {
                return Some(MarkdownContent::new(
...
pub fn render_memory_modal(
    ...
    let title = crate::locale::ctx().tr("Memory").into_owned();
...
fn build_shortcuts(state: &MemoryModalState) -> Vec<Shortcut<'static>> {
    ...
    let fullscreen_label = if state.fullscreen {
        crate::locale::ctx().tr_static("^F normal")
    } else {
        crate::locale::ctx().tr_static("^F fullscreen")
    };
    match &state.mode {
        MemoryModalMode::Browse if !state.memory_enabled => {
            let mut shortcuts = Vec::new();
            if state.can_enable() {
                shortcuts.push(plain(crate::locale::ctx().tr_static("t turn on")));
            }
            shortcuts.push(plain(fullscreen_label));
            shortcuts.push(plain(crate::locale::ctx().tr_static("Esc close")));
            shortcuts
        }
        MemoryModalMode::Browse if !state.has_notes() => {
            vec![
                plain(crate::locale::ctx().tr_static("t turn off")),
                plain(fullscreen_label),
                plain(crate::locale::ctx().tr_static("Esc close")),
            ]
        }
        MemoryModalMode::Browse => {
            let mut shortcuts = vec![
                plain(crate::locale::ctx().tr_static("\u{2191}/\u{2193} nav")),
                plain(crate::locale::ctx().tr_static("/ search")),
                plain(crate::locale::ctx().tr_static("y copy path")),
            ];
            if state.can_delete_selected() {
                shortcuts.push(plain(crate::locale::ctx().tr_static("x delete")));
            }
            shortcuts.extend([
                plain(crate::locale::ctx().tr_static("t turn off")),
                plain(fullscreen_label),
...
fn format_modified(epoch_secs: Option<u64>, now_secs: u64) -> String {
    ...
    let Some(modified) = epoch_secs else {
        return ctx.tr("unknown").into_owned();
    };
    if now_secs <= modified {
        return ctx.tr("just now").into_owned();
    }
    ...
    if delta < 3600 {
        ...
        return ctx.tr_format("{value}m ago", &[("value", &mins)]);
    }
    if delta < 86400 {
        let hours = (delta / 3600).to_string();
        return ctx.tr_format("{value}h ago", &[("value", &hours)]);
    }
    let days = (delta / 86400).to_string();
    ctx.tr_format("{value}d ago", &[("value", &days)])
}
