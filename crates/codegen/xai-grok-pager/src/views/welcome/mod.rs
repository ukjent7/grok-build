...
fn quit_hint_spans(theme: &Theme) -> Vec<Span<'static>> {
    ...
    vec![
        ...
        Span::styled(
            format!("  {}", crate::locale::ctx().tr_static("quit")),
...
pub(super) fn render_pending_hint(
    ...
    let line = Line::from(vec![
        ...
        Span::styled(
            crate::locale::ctx().tr_format("press again to {action}", &[("action", pending.label)]),
            action_style,
        ),
    ]);
...
pub(super) fn render_version_badge(
        ...
        } = &mode
    {
        spans.push(Span::styled(
            crate::locale::ctx().tr_format("Tier: {tier}", &[("tier", *tier)]),
            Style::default().fg(theme.gray),
        ));
        spans.push(sep.clone());
    }
    if show_api_key && is_api_key_auth {
        spans.push(Span::styled(
            crate::locale::ctx().tr_static("Logged in with API key"),
            Style::default().fg(theme.gray),
        ));
        spans.push(sep);
    }
    ...
    let channel = match xai_grok_update::channel_name() {
        Some("alpha") => format!(" [{}]", crate::locale::ctx().tr("alpha")),
        Some("stable") => format!(" [{}]", crate::locale::ctx().tr("stable")),
...
pub fn render_welcome(
    ...
    let mut result = match params.auth_state {
        AuthState::Pending { error } => {
            ...
            let login_text =
                crate::locale::ctx().tr_format("Login with {provider}", &[("provider", label)]);
            let quit = crate::locale::ctx().tr_static("Quit");
        ...
        AuthState::Done if params.is_zdr_blocked => {
            let quit = crate::locale::ctx().tr_static("Quit");
            let menu = [
                ("l", crate::locale::ctx().tr_static("Switch account")),
                ("q", quit),
            ];
            let (menu_rects, post_flush_escapes) = render_welcome_blocked(
                content_area,
                ...
                Some((
                    crate::locale::ctx()
                        .tr_static("Grok Build is not yet available for this account."),
                    theme.gray_bright,
                )),
                &menu,
                params.selected,
...
fn render_welcome_trust(
    ...
    let menu_items = [
        ("y", ctx.tr_static("Yes, proceed")),
        ("n", ctx.tr_static("No, quit")),
    ];
    let lines = vec![
        Line::from(Span::styled(
            ctx.tr_static("Do you trust the contents of this directory?"),
        ...
        Line::from(Span::styled(
            ctx.tr_static("Grok Build may run or modify contents in this directory,"),
            Style::default().fg(theme.gray),
        ))
        .alignment(Alignment::Center),
        Line::from(Span::styled(
            ctx.tr_static("posing security risks."),
            Style::default().fg(theme.gray),
        ))
        .alignment(Alignment::Center),
...
fn push_auth_copy_block(
    ...
    lines.push(match clipboard_delivery {
        Some(crate::clipboard::ClipboardDelivery::Confirmed) => Line::from(Span::styled(
            crate::locale::ctx().tr_static("copied!"),
            Style::default().fg(theme.gray),
        ))
        .alignment(Alignment::Center),
        ...
        Some(crate::clipboard::ClipboardDelivery::Failed) => Line::from(Span::styled(
            crate::locale::ctx().tr_static("copy failed"),
            Style::default().fg(theme.gray),
        ))
        .alignment(Alignment::Center),
        None => Line::default(),
...
fn render_raw_url_mode(
    ...
    let hint = Line::from(Span::styled(
        crate::locale::ctx().tr_static("Select the URL below with your mouse and copy manually."),
        Style::default().fg(theme.gray),
    ))
    .alignment(Alignment::Center);
    ...
    let hint_spans = vec![
        ...
        Span::styled(
            format!("  {}", crate::locale::ctx().tr_static("go back")),
            Style::default().fg(theme.gray),
        ),
    ];
...
fn render_browser_status_arm(
    ...
    let device_code_caption = ctx.tr_static(DEVICE_CODE_CAPTION);

    let header_rows = (unicode_width::UnicodeWidthStr::width(header) as u16).div_ceil(inner_width);
    let code_extra = if user_code.is_some() {
        let caption_rows = (unicode_width::UnicodeWidthStr::width(device_code_caption) as u16)
...
fn render_welcome_authenticating(
    ...
    match mode {
        AuthMode::Loopback => {
            ...
            let auth_header = crate::locale::ctx().tr_static(AUTH_HEADER);
            let msg_height = if auth_url.is_some() {
                let header_rows = (unicode_width::UnicodeWidthStr::width(auth_header) as u16)
                    .div_ceil(inner_width);
            ...
            if auth_url.is_some() {
                ...
                lines.push(
                    Line::from(Span::styled(
                        crate::locale::ctx().tr_static("Waiting for auth URL..."),
                        Style::default().fg(theme.gray),
                    ))
                    .alignment(Alignment::Center),
                );
            ...
            let mut hint_spans = vec![
                ...
                Span::styled(
                    format!("  {}    ", crate::locale::ctx().tr_static("submit")),
                    Style::default().fg(theme.gray),
                ),
            ];
        ...
        AuthMode::Pending => {
            ...
            let msg = Line::from(Span::styled(
                crate::locale::ctx().tr_static("Connecting..."),
                Style::default().fg(theme.gray_bright),
            ))
            .alignment(Alignment::Center);
...
fn render_changelog_section(
    ...
    let title = crate::locale::ctx().tr_static("Changelog");
    buf.set_span(
        centered.x,
        centered.y,
...
fn render_welcome_done(
    ...
    let cta = p
        ...
        .unwrap_or(ctx.tr_static("Upgrade Subscription"));
    ...
    let menu_items: &[(&str, &str)] = if !p.has_access {
        gate_menu = [
            (key_g, cta),
            (key_l, ctx.tr_static("Logout")),
            (key_q, ctx.tr_static("Quit")),
        ];
        &gate_menu
    } else {
        ...
        if p.has_claude_import {
            ...
            items.push((key_i_with_x, ctx.tr_static("Import Claude settings")));
        }
        items.push((key_w, ctx.tr_static("New worktree")));
        items.push((key_resume, ctx.tr_static("Resume session")));
        // "Changelog" above Quit; no shortcut, opened by click (row or block)
        if show_changelog_action {
            items.push(("", ctx.tr_static("Changelog")));
        }
        items.push((key_q, ctx.tr_static("Quit")));
        owned_menu = items;
        owned_menu.as_slice()
    };
    ...
    let (cursor_pos, post_flush_escapes) = if show_picker {
        ...
        let tier_prefix_label = ctx.tr_static("Tier: ");
        let refresh_text = ctx.tr_static("[Refresh]");
        ...
        let gate_text = match p.gate {
            ...
            None => crate::locale::ctx()
                .tr("SuperGrok subscription required")
                .into_owned(),
        };
        let msg = Line::from(Span::styled(
            gate_text,
            ...
            && layout.tip.height > 0
        {
            ...
            let line = Line::from(vec![
                Span::styled(
                    crate::locale::ctx().tr_static("Update: "),
                    Style::default()
                        .fg(theme.accent_user)
                        .add_modifier(Modifier::BOLD),
                ),
            ...
            && layout.tip.height > 0
        {
            ...
            let when = if mins == 0 {
                crate::locale::ctx().tr("moments ago").into_owned()
            } else {
                let minutes = mins.to_string();
                crate::locale::ctx().tr_format("{minutes}m ago", &[("minutes", &minutes)])
            };
            ...
            let coming_from = crate::locale::ctx().tr_static("Coming from ");
            let resume_text = crate::locale::ctx().tr_format(
...
pub(crate) fn render_session_picker_body(
        ...
        .enumerate()
    {
        ...
        picker_entries.push(PickerEntry::Row(PickerRow {
            ...
            badge: if has_snippet {
                crate::locale::ctx().tr_static("match")
            } else {
                ""
            },
    ...
    let config = PickerConfig {
        title: Some(crate::locale::ctx().tr_static("Resume session")),
