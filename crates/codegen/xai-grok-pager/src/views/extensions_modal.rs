...
impl ExtensionsTab {
    ...
    pub fn label(self) -> &'static str {
        match self {
            Self::Hooks => crate::locale::ctx().tr_static("Hooks"),
            Self::Plugins => crate::locale::ctx().tr_static("Plugins"),
            Self::Marketplace => {
                crate::locale::ctx().named_static_text("extensions.tab.marketplace", "Marketplace")
            }
            Self::Skills => crate::locale::ctx().tr_static("Skills"),
            Self::Workflows => crate::locale::ctx().tr_static("Workflows"),
            Self::McpServers => crate::locale::ctx().tr_static("MCP Servers"),
        }
    }
        ...
        match key {
            ...
            KeyEvent {
                ...
                if !empty_required.is_empty() {
                    self.error = Some(crate::locale::ctx().tr_format(
                        "Required: {fields}",
        ...
        match key.code {
            ...
            KeyCode::Enter => {
                if self.selected_value().is_none() {
                    self.error = Some(crate::locale::ctx().tr("Select an option").into_owned());
                    McpSetupOutcome::Changed
                } else {
    ...
    match (tab, ch) {
        ...
        (ExtensionsTab::Plugins, 'a') => Some(ButtonAction::StartInput {
            ...
            fields: vec![FieldSpec {
                ...
                placeholder: Some(
                    crate::locale::ctx()
                        .tr("owner/repo, URL, or local path")
                        .into_owned(),
                ),
            }],
        }),
        ...
        (ExtensionsTab::Marketplace, 'a') => Some(ButtonAction::StartInput {
            ...
            fields: vec![FieldSpec {
                ...
                placeholder: Some(
                    crate::locale::ctx()
                        .tr("owner/repo, git URL, or local path")
                        .into_owned(),
                ),
            }],
        }),
    ...
    let display = {
        if let Ok(rest) = source_path.strip_prefix(&grok) {
                ...
                && let Ok(rest) = source_path.strip_prefix(&home)
            {
                ...
                crate::locale::ctx().tr_format("Custom: {path}", &[("path", &source_dir)])
            }
        } else {
            crate::locale::ctx().tr_format("Custom: {path}", &[("path", &source_dir)])
        }
...
fn build_plugin_fields(plugin: &xai_hooks_plugins_types::PluginInfo) -> Vec<String> {
    ...
    if !plugin.skill_names.is_empty() {
        components.push(crate::locale::ctx().tr_format(
            "skills: {names}",
            &[("names", &plugin.skill_names.join(", "))],
        ));
    } else if plugin.skill_count > 0 {
        let count = plugin.skill_count.to_string();
        components.push(crate::locale::ctx().tr_format("{count} skills", &[("count", &count)]));
    }
    if !plugin.agent_names.is_empty() {
        ...
        components.push(crate::locale::ctx().tr_format("{count} agents", &[("count", &count)]));
    }
    if plugin.hook_count > 0 {
        let count = plugin.hook_count.to_string();
        components.push(crate::locale::ctx().tr_format("{count} hooks", &[("count", &count)]));
    }
    match plugin.mcp_status {
        McpStatus::Active | McpStatus::ActiveInline => {
            let count = plugin.mcp_server_count.to_string();
            components
                .push(crate::locale::ctx().tr_format("{count} MCP servers", &[("count", &count)]));
        }
        McpStatus::Blocked => {
            let count = plugin.mcp_server_count.to_string();
            components
                .push(crate::locale::ctx().tr_format("{count} MCP: blocked", &[("count", &count)]));
        }
        McpStatus::None => {}
    }
    components
    ...
    components.categories().map(|(category, items)| {
        let label = match category {
            ComponentCategory::Skills => {
                crate::locale::ctx().named_static_text("extensions.field.skills", "skills")
            }
            ComponentCategory::Commands => {
                crate::locale::ctx().named_static_text("extensions.field.commands", "commands")
            }
            ComponentCategory::Agents => crate::locale::ctx().tr_static("agents"),
            ComponentCategory::McpServers => crate::locale::ctx().tr_static("mcp servers"),
            ComponentCategory::Hooks => crate::locale::ctx().tr_static("hooks"),
            ComponentCategory::LspServers => crate::locale::ctx().tr_static("lsp servers"),
        };
        (label, items)
    })
}
...
pub(crate) fn render_components_fields(
    ...
    for (label, items) in component_categories(components) {
        ...
        if items.len() > COMPONENT_ITEMS_CAP {
            ...
            value.push_str(&crate::locale::ctx().tr_format(" +{count} more", &[("count", &more)]));
        }
...
pub fn render_extensions_modal(
        ...
        match state.active_tab {
            ExtensionsTab::Skills => {
                if let TabDataState::Loaded(ref skills) = state.skills_data {
                    ...
                    for group_label in &group_order {
                        ...
                        for m in members {
                                ...
                                && !tools.is_empty()
                            {
                                fields.push((
                                    crate::locale::ctx()
                                        .named_text("extensions.field.tools", "tools")
                                        .into_owned(),
                                    tools.join(", "),
                                ));
                            }
                            ...
                            if !skill.enabled {
                                entry_badge_text
                                    .push(crate::locale::ctx().tr("[disabled]").into_owned());
                    ...
                    entry_labels
                        .push(crate::locale::ctx().tr_format("Error: {error}", &[("error", msg)]));
                    entry_right_labels.push(String::new());
                    entry_desc_lines.push(vec![]);
                    entry_summary_lines.push(vec![]);
                    entry_fields.push(vec![]);
                ...
                if let TabDataState::Loaded(ref response) = state.plugins_data {
                    ...
                    for (group_key, plugins) in &groups {
                        ...
                        for &(pi, plugin) in plugins {
                            ...
                            {
                                fields.push((
                                    crate::locale::ctx().tr("description").into_owned(),
                                    desc.clone(),
                                ));
                            }
                            fields.push((
            ...
            ExtensionsTab::Hooks => {
                if let TabDataState::Loaded(ref data) = state.hooks_data {
                    ...
                    for group in &groups {
                        ...
                        let count_str = count.to_string();
                        entry_labels.push(crate::locale::ctx().tr_format(
                            "{source} ({count} hooks)",
                        ...
                        for &hi in indices {
                            ...
                            let no_command = crate::locale::ctx().tr("(no command)").into_owned();
                            let cmd = hook
                                .command
                                .as_deref()
                            ...
                            entry_badge_text.push(if hook.pinned {
                                crate::locale::ctx().tr("[policy]").into_owned()
                    ...
                    for view in ordered_marketplace_view(&data.sources) {
                        ...
                        for &pi in plugin_order {
                            ...
                            let status_label = match plugin.install_status.as_str() {
                                ...
                                "update_available" => {
                                    crate::locale::ctx().tr_static("[update available]")
                                }
                                _ => "",
                            };
                            entry_labels.push(plugin.name.clone());
                            let right =
                                match (plugin.version.as_deref(), plugin.author.as_deref()) {
                                    (Some(v), Some(a)) => crate::locale::ctx().tr_format(
                                        "v{version} by {author}",
                                        &[("version", v), ("author", a)],
                                    ),
                                    (Some(v), None) => crate::locale::ctx()
                                        .tr_format("v{version}", &[("version", v)]),
                                    (None, Some(a)) => crate::locale::ctx()
                                        .tr_format("by {author}", &[("author", a)]),
                            ...
                            match &plugin.components {
                                ...
                                Some(_) => {
                                    fields.push((
                                        crate::locale::ctx().tr("provides").into_owned(),
                                        crate::locale::ctx()
                                            .tr(NO_DETECTABLE_COMPONENTS)
                                            .into_owned(),
                                    ));
                                }
                                None => {
                                    if plugin.remote_url.is_some() {
                                        fields.push((
                                            crate::locale::ctx().tr("provides").into_owned(),
                                            crate::locale::ctx()
                                                .tr("contents shown after install")
                                                .into_owned(),
                                        ));
                                    }
                                }
                            ...
                            if plugin.install_status != "not_installed" {
                                ...
                                if let Some(ref iv) = plugin.installed_version {
                                    fields.push((
                                        crate::locale::ctx().tr("installed").into_owned(),
                                        iv.clone(),
                                    ));
                                }
                            }
                        ...
                        for &(si, server) in section_servers {
                            ...
                            if server.tools.is_empty() {
                                let line =
                                    if server.status == McpServerDisplayStatus::BlockedByPolicy {
                                        ...
                                        match server.blocked_reason.as_deref() {
                                            Some(reason) => crate::locale::ctx().tr_format(
                                                "blocked by policy — {reason}",
                                                &[("reason", reason)],
                                            ),
                                            None => crate::locale::ctx()
                                                .tr("blocked by policy")
                                                .into_owned(),
                                        }
                                    } else if !server.enabled {
                                        crate::locale::ctx()
                                            .tr("no tools — server is disabled")
                                            .into_owned()
                                    } else if matches!(
                                        server.status,
                                        ...
                                        crate::locale::ctx()
                                            .tr("no tools (server may not be connected)")
                                            .into_owned()
                                    };
                                ...
                                if enabled_count == server.tools.len() {
                                    entry_desc_lines.push(vec![
                                        crate::locale::ctx()
                                            .tr_format("{count} tools", &[("count", &total)]),
                                    ]);
                                } else {
                                    let enabled = enabled_count.to_string();
                                    entry_desc_lines.push(vec![crate::locale::ctx().tr_format(
    ...
    {
        let processing = crate::locale::ctx().tr("Processing...").into_owned();
        let label = state
            .pending_action
            .as_deref()
            .unwrap_or(processing.as_str());
    ...
    if let Some(kind) = modal_msg_kind {
        let segments: &[(&str, &str)] = match kind {
            ...
            ModalMsgKind::Confirm => &[
                ("y", crate::locale::ctx().tr_static(" confirm")),
                (
